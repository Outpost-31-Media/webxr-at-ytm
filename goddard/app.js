...
async function loadModels(lod) {
    ...
    const models = ["./gltf/goddardDesktop.glb", "./gltf/goddardDesktop.glb", "./gltf/goddardDesktop.glb"];
    const reticleURLs = ["./gltf/goddardDesktopReticle.glb","./gltf/goddardDesktopReticle.glb","./gltf/goddardDesktopReticle.glb"]
...
function shipLoaderCallback() {
    shipMixer = new THREE.AnimationMixer(ship);
    // console.log(modelGltf.animations);

    const shipAnimKeys = [...Array(gltfShip.animations.length).keys()];
    console.log(gltfShip.animations);

    shipAnimKeys.forEach(key => {
        const anim = shipMixer.clipAction(gltfShip.animations[key]);
        shipAnims.push(anim);
    })
}
